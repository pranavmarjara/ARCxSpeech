...
def save_assessment(
    ...
    assessment = {
        ...
        # Shared identifier linking this full assessment record to its
        # lightweight "ui_tasks" stub counterpart in the same file (see
        # api/routes.py save_session / create_session_from_upload) --
        # used so a session can be deleted as a single unit instead of
        # relying on array position, which shifts once entries are
        # removed.
        "session_id": session_id,

        # "live" (recorded from ARC hardware) or "uploaded" (built from
        # uploaded .wav files via the R&D upload path). Lets the UI show
        # a Live/Uploaded label per session.
